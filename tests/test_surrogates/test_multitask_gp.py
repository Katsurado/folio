...
class TestMultiTaskGPSurrogateDtype:
    """Test MultiTaskGPSurrogate dtype validation."""

    def test_fit_float32_x_raises(self):
        """fit() raises ValueError for float32 X."""
        X = np.array([[0.0], [0.5], [1.0]], dtype=np.float32)
        y = np.array([[0.0, 0.1], [0.5, 0.6], [1.0, 1.1]], dtype=np.float64)
    ...
    def test_fit_float32_y_raises(self):
        ...
        y = np.array([[0.0, 0.1], [0.5, 0.6], [1.0, 1.1]], dtype=np.float32)
        with pytest.raises(ValueError, match="float64"):
            MultiTaskGPSurrogate().fit(X, y)
